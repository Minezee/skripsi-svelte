  ...
  <div class="container py-3 flex flex-row justify-between gap-4 items-center">
    ...
    <div class="flex flex-row items-center gap-3 lg:gap-5">
      ...
      {#if $auth.token}
        ...
        >
          <ShoppingCart size={24} class="text-gray-700" />
          {#if cart.items.length > 0}
            <div
              class="absolute -right-1 -top-1 bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs"
            >
              {cart.items.length}
            </div>
          {/if}
